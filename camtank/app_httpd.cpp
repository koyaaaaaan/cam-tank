...
static esp_err_t gpio_digital_read_handler(httpd_req_t *req)
{    
    char *buf = NULL;
    if (parse_get(req, &buf) != ESP_OK) {
        return ESP_FAIL;
    }
    int pin = parse_get_var(buf, "pin", 0);
    int val = digitalRead(pin);

    char *respOK = NULL;
    sprintf(respOK, "%d", val);
    return httpd_resp_send(req, respOK, sizeof(respOK));
}

static esp_err_t gpio_digital_write_handler(httpd_req_t *req)
{    
    Serial.println("gpio_servo_angle_handler");
    char *buf = NULL;
    if (parse_get(req, &buf) != ESP_OK) {
        return ESP_FAIL;
    }

    char pinName[5][5] = {"pin1","pin2","pin3","pin4","pin5"};
    char valName[5][5] = {"val1","val2","val3","val4","val5"};
    for(int i=0;i<5;i++){
      int pin = parse_get_var(buf, pinName[i], -1);
      int val = parse_get_var(buf, valName[i], -1);
      if(pin == -1 || val == -1){
        continue;
      }
      if(val == 0){
...
static esp_err_t gpio_servo_angle_handler(httpd_req_t *req)
{
    Serial.println("gpio_servo_angle_handler");
    /* # Servo
    char *buf = NULL;
...
void setServo(const int svBASEPIN, const int svCAMPIN){
  svBASE.attach(svBASEPIN);
  svCAM.attach(svCAMPIN);
}
...
void startCameraServer()
{
    ...
    // For GPIO Contraol
    httpd_uri_t gpio_dr_url = {
        .uri = "/gpio-digital-read",
        .method = HTTP_GET,
        .handler = gpio_digital_read_handler,
        .user_ctx = NULL};

    httpd_uri_t gpio_dw_url = {
        .uri = "/gpio-digital-write",
        .method = HTTP_GET,
        .handler = gpio_digital_write_handler,
        .user_ctx = NULL};

    httpd_uri_t gpio_servo_url = {
        .uri = "/gpio-servo-angle",
        .method = HTTP_GET,
        .handler = gpio_servo_angle_handler,
        .user_ctx = NULL};
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
        ...
        // For GPIO Control
        httpd_register_uri_handler(camera_httpd, &gpio_dr_url);
        httpd_register_uri_handler(camera_httpd, &gpio_dw_url);
        httpd_register_uri_handler(camera_httpd, &gpio_servo_url);
    }
